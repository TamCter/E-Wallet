...
export default function TransferScreen() {
  ...
  const handlePhoneLookup = () => {
    if (!phoneCountryCode) {
      Alert.alert('Lỗi', 'Vui lòng nhập mã quốc gia');
      return;
    }
    if (phone.length < 9) {
  ...
  const renderPhoneTab = () => (
    <View style={styles.card}>
      ...
      >
        <Text style={styles.primaryBtnText}>{isLookingUp ? 'Đang tìm kiếm...' : 'Tiếp tục →'}</Text>
      </TouchableOpacity>

      {/* Contacts */}
      <Text style={[styles.cardTitle, { marginTop: 24 }]}>Gần đây</Text>
      {[
        { name: 'Trần Thị B', phone: '0912 345 678' },
        { name: 'Lê Văn C', phone: '0987 654 321' },
        { name: 'Phạm Thị D', phone: '0901 234 567' },
      ].map((c, i) => (
        <TouchableOpacity
          key={i}
          style={styles.contactRow}
          onPress={() => { setPhone(c.phone.replace(/\s/g, '')); setRecipientName(c.name); setStep('amount'); }}
        >
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.balanceHint}>Số dư khả dụng: {formatCurrency(String(MOCK_BALANCE))}đ</Text>
  ...
  const renderReviewStep = () => (
    <ScrollView contentContainerStyle={styles.body}>
      <View style={styles.card}>
        ...
        <Text style={styles.reviewNote}>🔒 Giao dịch được bảo vệ bởi mã hóa 256-bit</Text>
